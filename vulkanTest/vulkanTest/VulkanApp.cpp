...
void VulkanApp::loadModel() {
  ...
  if (!tinyobj::LoadObj(&attrib, &shapes, &materials_t, &warn, &err,
                        MODEL_PATH.c_str(), BASE_PATH.c_str())) {
  ...
  for (const auto &shape : shapes) {
    ...
    for (const auto &index : shape.mesh.indices) {
      ...
      if (index_counter % 3 == 0) {
        materialsIndex.push_back(
            shape.mesh.material_ids[material_index_counter]);
      }
      index_counter++;
    }
  }
  ...
  for (const auto &material : materials_t) {
    Material m;
    m.diffuse = glm::vec3(material.diffuse[0], material.diffuse[1],
                          material.diffuse[2]);
    m.specular = glm::vec3(material.specular[0], material.specular[1],
                           material.specular[2]);
...
void VulkanApp::createTextureImage() {
  ...
  createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
               ...
               0, stagingBuffer, stagingBufferMemory);
...
void VulkanApp::createUniformBuffers() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                 0, uniformBuffers[i], uniformBuffersMemory[i]);
...
void VulkanApp::createIndexBuffer() {
  ...
  createBuffer(
      ...
      VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, indexBuffer, indexBufferMemory);
...
void VulkanApp::createVertexBuffer() {
  ...
  createBuffer(
      ...
      VK_BUFFER_USAGE_TRANSFER_DST_BIT |
          ...
          VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
      VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, vertexBuffer, vertexBufferMemory);
  ...
  */
  ...
  createBuffer(
      ...
      VK_BUFFER_USAGE_TRANSFER_DST_BIT |
          ...
          VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
      VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, matIndexBuffer.buffer,
      matIndexBuffer.memory);
  ...
  vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
  memcpy(data, reinterpret_cast<const uint8_t *>(materials.data()),
         (size_t)bufferSize);
  vkUnmapMemory(device, stagingBufferMemory);
  ...
  createBuffer(
      ...
      VK_BUFFER_USAGE_TRANSFER_DST_BIT |
          ...
          VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
      VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, matColorBuffer.buffer,
      matColorBuffer.memory);
...
void VulkanApp::bindPipeline(VkCommandBuffer commandBuffer,
                             VkPipeline pipeline) {
