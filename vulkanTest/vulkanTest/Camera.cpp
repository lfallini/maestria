...
float Camera::get_near_clip() { return znear; }

float Camera::get_far_clip() { return zfar; }
...
bool Camera::update_gamepad(glm::vec2 axis_left, glm::vec2 axis_right,
                            float delta_time) {
  bool changed = false;
  ...
  if (type == CameraType::FirstPerson) {
    ...
    if (fabsf(axis_left.y) > dead_zone) {
      ...
      position -=
          front * pos * ((axis_left.y < 0.0f) ? -1.0f : 1.0f) * move_speed;
      changed = true;
    }
    if (fabsf(axis_left.x) > dead_zone) {
      float pos = (fabsf(axis_left.x) - dead_zone) / range;
      position +=
          glm::normalize(glm::cross(front, glm::vec3(0.0f, 1.0f, 0.0f))) * pos *
    ...
    if (fabsf(axis_right.x) > dead_zone) {
      ...
      rotation.y +=
          pos * ((axis_right.x < 0.0f) ? -1.0f : 1.0f) * new_rotation_speed;
      changed = true;
    }
    if (fabsf(axis_right.y) > dead_zone) {
      float pos = (fabsf(axis_right.y) - dead_zone) / range;
      rotation.x -=
          pos * ((axis_right.y < 0.0f) ? -1.0f : 1.0f) * new_rotation_speed;
      changed = true;
    }
  } else {
