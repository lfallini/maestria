...
void RtVulkanApp::createStorageImage() {
  ...
  createImage(width, height, VK_FORMAT_B8G8R8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_STORAGE_BIT,
...
void RtVulkanApp::createBottomLevelAS() {
  ...
  pfnGetAccelerationStructureBuildSizesKHR(device, VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR, &accelerationStructureBuildGeometryInfo,
                                           &primitiveCount, &accelerationStructureBuildSizesInfo);

  // Create a buffer to hold the acceleration structure
  createBuffer(accelerationStructureBuildSizesInfo.accelerationStructureSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, 0, 0,
               bottomLevelAccelerationStructure.buffer, bottomLevelAccelerationStructure.memory);
  ...
  createBuffer(accelerationStructureBuildSizesInfo.buildScratchSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, scratchBuffer, scratchBufferMemory);
  ...
  pfnCmdBuildAccelerationStructuresKHR(commandBuffer, 1, &accelerationBuildGeometryInfo, accelerationBuildStructureRangeInfos.data());
...
void RtVulkanApp::updateInstancesBuffer(VkAccelerationStructureInstanceKHR accelerationStructureInstance, VkBuffer instancesBuffer) {
  VkDeviceSize bufferSize = sizeof(VkAccelerationStructureInstanceKHR);

  VkBuffer stagingBuffer;
  VkDeviceMemory stagingBufferMemory;
  createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, 0,
...
void RtVulkanApp::createTopLevelAS() {
  ...
  updateInstancesBuffer(accelerationStructureInstance, instancesBuffer);
  ...
  // Create a buffer to hold the acceleration structure
  createBuffer(accelerationStructureBuildSizesInfo.accelerationStructureSize, VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, 0,
               VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, topLevelAccelerationStructure.buffer, topLevelAccelerationStructure.memory);
...
void RtVulkanApp::createShaderBindingTable() {
  ...
  */
  ...
  const uint32_t handleSizeAligned =
      alignedSize(rayTracingPipelineProperties.shaderGroupHandleSize, rayTracingPipelineProperties.shaderGroupHandleAlignment);
  ...
  createBuffer(handleSizeAligned * rgenIndex.size(), sbtBufferUsageFlags, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, raygenShaderBindingTable.buffer, raygenShaderBindingTable.memory);
  createBuffer(handleSizeAligned * missIndex.size(), sbtBufferUsageFlags, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, missShaderBindingTable.buffer, missShaderBindingTable.memory);
  createBuffer(handleSizeAligned * hitIndex.size(), sbtBufferUsageFlags, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
               VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, hitShaderBindingTable.buffer, hitShaderBindingTable.memory);
...
void RtVulkanApp::createDescriptorSets() {
  ...
  std::vector<VkWriteDescriptorSet> writeDescriptorSets = {accelerationStructureWrite, resultImageWrite, uniformBufferWrite, sceneBufferWrite};
...
void set_image_layout(VkCommandBuffer command_buffer, VkImage image, VkImageLayout old_layout, VkImageLayout new_layout,
                      VkImageSubresourceRange subresource_range, VkPipelineStageFlags src_mask = VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
...
void RtVulkanApp::buildCommandBuffers(uint32_t imageIndex) {
  ...
  for (int32_t i = 0; i < commandBuffers.size(); ++i) {
    ...
    */
    ...
    pfnCmdTraceRaysKHR(commandBuffers[i], &raygenShaderSbtEntry, &missShaderSbtEntry, &hitShaderSbtEntry, &callableShaderSbtEntry, appSettings.width,
                       appSettings.height, 1);
    ...
    */
    ...
    set_image_layout(commandBuffers[i], swapChainImages[imageIndex], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                     subresourceRange);

    // Prepare ray tracing output image as transfer source
    set_image_layout(commandBuffers[i], storageImage.image, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, subresourceRange);
    ...
    vkCmdCopyImage(commandBuffers[i], storageImage.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, swapChainImages[imageIndex],
                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &copyRegion);

    // Transition swap chain image back for presentation
    set_image_layout(commandBuffers[i], swapChainImages[imageIndex], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
                     subresourceRange);
...
void RtVulkanApp::createTransformMatrixBuffer() {
  ...
  createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
               VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, stagingBuffer, stagingBufferMemory);
...
void RtVulkanApp::createBufferReferences() {
  ...
  createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
               VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT, sceneDesc.buffer, sceneDesc.memory);
